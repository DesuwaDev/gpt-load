...
const ID0026 = "0026_auto_model"

var autoModelColumns0026 = []string{"AutoDecision", "DecisionModel", "DecisionCostNanoUSD", "DecisionPricingCompleteness"}

// 冻结本次迁移的字段，后续运行模型变化不能改写历史 DDL。
type autoLog0026 struct {
...
func (autoLog0026) TableName() string { return "request_logs" }

type autoUsage0026 struct {
...
func (autoUsage0026) TableName() string { return "auto_decision_usage_stats" }

func Up0026(db *gorm.DB) error {
	if err := ValidateRecoverable0026(db); err != nil {
		return err
	}
	for _, name := range autoModelColumns0026 {
		if !db.Migrator().HasColumn(&autoLog0026{}, name) {
			if err := db.Migrator().AddColumn(&autoLog0026{}, name); err != nil {
				return fmt.Errorf("add automatic model request log column %s: %w", name, err)
			}
		}
	}
	for _, table := range []any{&autoUsage0026{}} {
...
func Validate0026(db *gorm.DB) error {
	...
	for _, definition := range []struct {
		...
		{&autoUsage0026{}, []string{"BucketStartMS", "AccessKeyID", "Model", "EstimatedCostNanoUSD", "UnpricedRequestCount", "PricingPartialCount"}},
	} {
