...
const ID0025 = "0025_request_log_operation_index"
const requestLogOperationIndex0025 = "idx_request_logs_operation_completed_id"

type requestLog0025 struct {
...
func (requestLog0025) TableName() string { return "request_logs" }

type requestLogOperationIndexColumn0025 struct {
	Name       string
	Descending bool
	IsUnique   bool
	IsValid    bool
...
// Up0025 为操作筛选增加与游标顺序一致的索引，不改历史日志。
func Up0025(db *gorm.DB) error {
	if err := ValidateRecoverable0025(db); err != nil {
		return err
	}
	if !db.Migrator().HasIndex(&requestLog0025{}, requestLogOperationIndex0025) {
		if err := db.Migrator().CreateIndex(&requestLog0025{}, requestLogOperationIndex0025); err != nil {
...
func ValidateRecoverable0025(db *gorm.DB) error {
	if !db.Migrator().HasTable(&requestLog0025{}) {
...
func Validate0025(db *gorm.DB) error {
	columns, err := requestLogOperationIndexColumns0025(db)
	if err != nil {
		return err
	}
	want := []struct {
...
func requestLogOperationIndexColumns0025(db *gorm.DB) ([]requestLogOperationIndexColumn0025, error) {
	var columns []requestLogOperationIndexColumn0025
	var query string
	var arguments []any
	...
	case "postgres":
		...
		arguments = []any{requestLogOperationIndex0025}
	case "mysql":
		query = `
	...
	case "sqlite":
		...
		arguments = []any{requestLogOperationIndex0025, requestLogOperationIndex0025}
	default:
