...
const ID0030 = "0030_rpm_stats"

type rpmStat0030 struct {
...
func (rpmStat0030) TableName() string { return "rpm_stats" }

// Up0030 使用冻结模型扩展三数据库的同一增量链。
func Up0030(db *gorm.DB) error {
	if Validate0030(db) == nil {
		return nil
	}
	if err := ValidateRecoverable0030(db); err != nil {
		return err
	}
	if err := db.AutoMigrate(&rpmStat0030{}); err != nil {
...
func ValidateRecoverable0030(db *gorm.DB) error {
	if !db.Migrator().HasTable(&rpmStat0030{}) {
		return nil
	}
	if Validate0030(db) == nil {
		return nil
	...
	var count int64
	if err := db.Model(&rpmStat0030{}).Count(&count).Error; err != nil {
		return err
	}
	if count != 0 {
	...
	if err := statement.Parse(&rpmStat0030{}); err != nil {
		return err
	}
	columns, err := db.Migrator().ColumnTypes(&rpmStat0030{})
	if err != nil {
		return err
	}
