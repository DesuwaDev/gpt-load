...
const ID0024 = "0024_credential_quota_history"

type quotaHistory0024 struct {
...
func (quotaHistory0024) TableName() string { return "credential_quota_histories" }

// Up0024 使用冻结模型扩展三数据库的同一增量链。
func Up0024(db *gorm.DB) error {
	if Validate0024(db) == nil {
		return nil
	}
	if err := ValidateRecoverable0024(db); err != nil {
		return err
	}
	if err := db.AutoMigrate(&quotaHistory0024{}); err != nil {
...
func ValidateRecoverable0024(db *gorm.DB) error {
	if !db.Migrator().HasTable(&quotaHistory0024{}) {
		return nil
	}
	if Validate0024(db) == nil {
		return nil
	...
	var count int64
	if err := db.Model(&quotaHistory0024{}).Count(&count).Error; err != nil {
		return err
	}
	if count != 0 {
	...
	if err := statement.Parse(&quotaHistory0024{}); err != nil {
		return err
	}
	columns, err := db.Migrator().ColumnTypes(&quotaHistory0024{})
	if err != nil {
		return err
	}
