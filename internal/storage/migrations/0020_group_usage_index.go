...
const ID0020 = "0020_group_usage_index"
const requestLogGroupIndex0020 = "idx_request_logs_group_completed"

type requestLog0020 struct {
...
func (requestLog0020) TableName() string { return "request_logs" }

// Up0020 只新增精确窗口查询索引，不改日志、用量或转发语义。
func Up0020(db *gorm.DB) error {
	if err := ValidateRecoverable0020(db); err != nil {
		return err
	}
	if db.Migrator().HasIndex(&requestLog0020{}, requestLogGroupIndex0020) {
		return nil
	}
	return db.Migrator().CreateIndex(&requestLog0020{}, requestLogGroupIndex0020)
}

func ValidateRecoverable0020(db *gorm.DB) error {
	if !db.Migrator().HasTable(&requestLog0020{}) {
