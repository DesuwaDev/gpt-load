...
const ID0028 = "0028_client_model_overrides"

type clientModelOverride0028 struct {
	ModelHash   string              `gorm:"type:varchar(64);primaryKey;not null"`
	ClientModel clientModelText0028 `gorm:"not null"`
	Overrides   clientModelText0028 `gorm:"not null"`
}

func (clientModelOverride0028) TableName() string { return "client_model_overrides" }

type clientModelText0028 string

func (clientModelText0028) GormDBDataType(db *gorm.DB, _ *schema.Field) string {
...
func Up0028(db *gorm.DB) error {
	if db.Migrator().HasTable(&clientModelOverride0028{}) {
	...
	if err := db.Migrator().CreateTable(&clientModelOverride0028{}); err != nil {
