...
const ID0029 = "0029_request_audit"

type auditLog0029 struct {
...
func (auditLog0029) TableName() string { return "request_logs" }

type auditParent0029 struct {
	ID string `gorm:"type:varchar(36);primaryKey;not null"`
}

func (auditParent0029) TableName() string { return "request_logs" }

type auditUsage0029 struct {
	Request              auditParent0029 `gorm:"foreignKey:RequestID;references:ID;constraint:OnDelete:CASCADE"`
...
func (auditUsage0029) TableName() string { return "request_audit_usages" }

var auditColumns0029 = []string{"RequestAudit", "AuditCostNanoUSD", "AuditPricingCompleteness"}

func Up0029(db *gorm.DB) error {
	if err := ValidateRecoverable0029(db); err != nil {
		return err
	}
	for _, name := range auditColumns0029 {
		if !db.Migrator().HasColumn(&auditLog0029{}, name) {
			if err := db.Migrator().AddColumn(&auditLog0029{}, name); err != nil {
				return err
			}
		}
	}
	if !db.Migrator().HasTable(&auditUsage0029{}) {
		if err := db.Migrator().CreateTable(&auditUsage0029{}); err != nil {
			return err
		}
	}
	if !db.Migrator().HasIndex(&auditUsage0029{}, "idx_request_audit_usage_completed") {
		if err := db.Migrator().CreateIndex(&auditUsage0029{}, "idx_request_audit_usage_completed"); err != nil {
			return err
		}
	}
	if !db.Migrator().HasConstraint(&auditUsage0029{}, "Request") {
		if err := db.Migrator().CreateConstraint(&auditUsage0029{}, "Request"); err != nil {
			return err
		}
	}
...
func ValidateRecoverable0029(db *gorm.DB) error {
	...
	if db.Migrator().HasTable(&auditUsage0029{}) {
		return validateAuditTable0029(db, false)
	}
	return nil
}
func Validate0029(db *gorm.DB) error {
	...
	return validateAuditTable0029(db, true)
}
func validateAuditTable0029(db *gorm.DB, requireIndex bool) error {
	if !db.Migrator().HasTable(&auditUsage0029{}) {
	...
	for _, name := range []string{"RequestID", "Sequence", "CompletedAtMS", "AccessKeyID", "GroupID", "ChannelID", "CredentialID", "Model", "EstimatedCostNanoUSD", "PricingCompleteness", "UnpricedRequestCount", "PricingPartialCount"} {
		if !db.Migrator().HasColumn(&auditUsage0029{}, name) {
			return fmt.Errorf("audit usage column %s missing", name)
		}
	}
	if requireIndex && !db.Migrator().HasConstraint(&auditUsage0029{}, "Request") {
		return fmt.Errorf("audit usage request constraint missing")
	}
	if requireIndex && !db.Migrator().HasIndex(&auditUsage0029{}, "idx_request_audit_usage_completed") {
