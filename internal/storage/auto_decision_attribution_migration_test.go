...
func TestAutoDecisionAttributionMigrationCleansInterruptedSwapBackup(t *testing.T) {
	...
	if err := db.Create(&schemaMigration{ID: migrationResumeMarker(migrations[26].ID)}).Error; err != nil {
		t.Fatal(err)
	}
	if err := migrations[26].Up(db); err != nil {
		t.Fatal(err)
	}
	const backupTable = "auto_decision_usage_stats_0027_old"
	if err := db.Exec(`CREATE TABLE auto_decision_usage_stats_0027_old AS
	...
	for _, scenario := range []string{"fresh", "upgrade", "interrupted"} {
		t.Run(scenario, func(t *testing.T) {
			...
			if scenario == "interrupted" {
				registry := append([]migration(nil), migrations[:27]...)
				up := registry[26].Up
				registry[26].Up = func(tx *gorm.DB) error {
