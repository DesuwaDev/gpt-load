...
func TestProductionRawSQLDoesNotUseReservedSchemaIdentifiers(t *testing.T) {
	...
	if got := strings.Join(sortedKeys(reservedIdentifiers), ","); got != "groups,key" && got != "groups,key,trigger" {
		t.Fatalf("reserved schema identifiers = %q, want %q", got, "groups,key,trigger")
	}
