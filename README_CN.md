...
- `cache_key_rotation_enabled` 默认关闭，分组显式设置优先于全局，未覆盖时继承全局。开启后，当降智检测判定当前凭据以次充好、或上游因容量不足在处理前拒绝了这次尝试时，网关会把该会话换到另一个上游缓存分片；一个降智周期内同一会话只换一次。代价是这次对话已建立的上游缓存与服务商侧的推理续写会一并作废，换后第一轮延迟更高、计费的输入 token 更多，因此它是给已经降智的会话止损，不是常开的优化。
