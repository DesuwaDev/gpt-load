  ...
  settings: {
    ...
    runtime: {
      ...
      emptyResponseRetryHelp:
        'ストリーミングの対話リクエストにのみ適用します。上流が正常に完了しても内容を一切生成しなかった場合は失敗として次の候補で再試行します。認証情報のクールダウンやブラックリスト登録は行いません。再試行を使い切った場合でも、上流が返した空の応答をそのままクライアントへ渡します。プリウォームリクエスト、会話の継続リクエスト、WebSocket は対象外です。',
