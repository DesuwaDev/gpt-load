  ...
  group: {
    ...
    credentials: {
      ...
      turnState: {
        ...
        ttlRemaining: '剩余 {duration}',
        ttlExpired: '已超时 {duration}',
        ttlUnknown: '时效未知',
        ttlIssuedAt: '签发于 {time}（从值里的 Fernet 时间戳读出）',
        ttlApprox: '这个值不是标准 Fernet token，读不到签发时间，只能按保存时刻估算。',
        ttlUnknownHint: '这个值不是标准 Fernet token，读不到签发时间。',
        ttlNotice: '注入值已超过 1 小时时效，建议换一个；这里只是提醒，注入不会停。',
        apply: '保存',
        clear: '清除',
      },
