...
export type DegradationSettingsUpdateRequest = Partial<
  Omit<DegradationSettingsDto, 'updated_at_ms'>
>
...
export function projectDegradationMonitor(value: unknown): DegradationMonitorDto {
  ...
  const result: DegradationMonitorDto = {
    ...
    run_when_disabled:
      record.run_when_disabled === null ? null : projectBoolean(record.run_when_disabled),
    enabled: projectBoolean(record.enabled),
    ...
    effective_min_probability_micros: projectSafeInteger(record.effective_min_probability_micros, {
      minimum: 1,
      maximum: probabilityMicrosScale,
    }),
...
export function degradationMonitorCollectionQueryOptions(
  ...
  return queryOptions({
    ...manualDegradationQueryOptions,
    queryKey: computed(() => controlQueryKeys.degradation.monitors(toValue(filters))),
