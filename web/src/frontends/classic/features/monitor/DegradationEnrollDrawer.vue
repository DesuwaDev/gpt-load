...
const credentialsQuery = useQuery({
  ...
  enabled: computed(() => props.open && mode.value === 'credential' && credentialGroupId.value > 0),
    ...
    <form class="degradation-enroll" @submit.prevent="submit">
      <section class="degradation-enroll__section">
        ...
        <SearchableMultiSelect
          ...
          :remove-label="
            (label: string) => t('monitor.degradation.enroll.removeGroup', { name: label })
          "
      ...
      <section v-if="result && result.rejected.length" class="degradation-enroll__section">
        ...
        <ul class="degradation-enroll__rejected">
          <li
            v-for="(entry, index) in result.rejected"
            :key="`${entry.group_id}-${entry.credential_id}-${index}`"
          >
