...
- `cache_key_rotation_enabled` defaults to disabled. An explicit group setting overrides the global value; otherwise the group inherits it. When enabled, the gateway moves a conversation to a different upstream cache shard once degradation detection judges the selected credential to be serving that model degraded, or the upstream refuses an attempt for want of capacity before processing it. A conversation moves at most once per degradation episode. The move discards both the upstream cache and the provider-side reasoning that conversation had built up, so the first turn after a rotation is slower and bills more input tokens. It is a way to cut losses on an already degraded conversation, not an optimization to leave on.
